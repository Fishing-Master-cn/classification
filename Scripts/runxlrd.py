#!d:\force_predict\scripts\python.exe
...
if __name__ == "__main__":
    ...
    def show_row(bk, sh, rowx, colrange, printit):
        ...
        if bk.formatting_info:
            for colx, ty, val, cxfx in get_row_data(bk, sh, rowx, colrange):
                if printit:
                    print("cell %s%d: type=%d, inside_force_pre_data: %r, xfx: %s"
                        % (xlrd.colname(colx), rowx+1, ty, val, cxfx))
        else:
            for colx, ty, val, _unused in get_row_data(bk, sh, rowx, colrange):
                if printit:
                    print("cell %s%d: type=%d, inside_force_pre_data: %r" % (xlrd.colname(colx), rowx+1, ty, val))
    ...
    def bk_header(bk):
        ...
        print("Number of inside_force_pre_data sheets: %d" % bk.nsheets)
    ...
    def print_labels(sh, labs, title):
        ...
        for rlo, rhi, clo, chi in labs:
            print("%s output_feature range %s:%s contains:"
